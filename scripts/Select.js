...
class Select extends BaseComponent {
    ...
    constructor(rootElement) {
        ...
        // console.log(this.originalControlElement);
    ...
    updateUI() {
        ...
        const newSelectedOptionValue = selectedOptionElement.textContent.trim(); // убирает у получаемой строки слева и справа все пробелы
        ...
        const updateButton = () => {
            ...
            this.buttonElement.setAttribute(
                ...
                this.optionElements[currentOptionIndex].id // обращаемся к списку DOM-элемента опций, затем по определенному индексу получаем ссылку на конкретный DOM-элемент текущей опции, и читаем значение аттрибута id
            );
        };
    ...
    fixDropdownPosition() {
        ...
        const { width, x } = this.buttonElement.getBoundingClientRect(); // получаем размеры кнопки

        const buttonCenterX = x + width / 2; // координаты центра кнопки по оси X

        const isButtonOnTheLeftViewportSide = buttonCenterX < halfViewportX; // находится ли кнопка в левой части vw
        ...
        // в зависимости от положения кнопки меняем нужный класс
    }
    ...
    onClick = (e) => {
        ...
        if (!isButtonClick && isOutsideDropdownClick) {
            ...
            return; // чтобы завершить выполнение метода
        }
    ...
    onArrowUpKeyDown = () => {
        ...
        if (this.state.currentOptionIndex > 0) {
            this.state.currentOptionIndex--; // уменьшаем на 1
        }
    };
    ...
    onArrowDownKeyDown = () => {
        ...
        if (this.state.currentOptionIndex < this.optionElements.length - 1) {
            this.state.currentOptionIndex++; // увеличиваем на 1
        }
    };
    ...
    onKeyDown = (e) => {
        ...
        }[code]; // так в action у нас будет хранится либо ссылка на конкретный метод обработчик нажатия на соответствующую ему клавишу клавиатуры, либо будет undefined
    ...
    bindEvents() {
        ...
        ); // для синхронизации кастомного и обычного селекта
    }
}
